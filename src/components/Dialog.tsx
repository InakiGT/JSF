...
function Dialog({ title, subtitle, content, kind, modalRef }: DialogType) {
  ...
  return (
    <dialog ref={ modalRef } className="w-100 max-h-[98vh] bg-white text-[#212529] p-5 rounded-lg shadow-lg z-50 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 backdrop:bg-[rgba(0,0,0,0.4)] xl:max-h-[95vh] md:w-250 xl:p-10">
      ...
      <div className="flex gap-10 flex-wrap justify-center">
        {
          content.map((item, index) => (
            <article key={ index } className="border-2 border-[#00000020] rounded-xl xl:max-w-8/17 min-h-fit flex flex-col justify-between">
              <div className="p-6">
              ...
              <div className="border-t-2 border-[#00000020] bg-[#00000008] py-3 px-5">
                <button
                  ...
                  onClick={ () => item.link?.startsWith('http') ? window.open(item.link, '_blank') : navigate(item.link ? item.link : '#') }
                >Ir</button>
              </div>
            </article>
          ))
